...
void VRWidget::update()
{
    ...
    if (animationOn) {
        ...
        // Bouger la caméra en fonction de la lumière
        if (animationCamera){
            QVector3D newPosCam = QVector3D(2*light->getPositionLight().x(),2*light->getPositionLight().y(),2*light->getPositionLight().z());
            camera->setPosition(newPosCam);
        }
        else{
            camera->setRho(20);
            camera->setPhi(M_PI_4);
            camera->setTheta(0);
        }
        // Cibler la caméra sur la planète
        if (targetCamera){
            camera->setTarget(world->getPosition());
        }
        else{
...
void VRWidget::startAnimationCamera()
{
    animationCamera = !animationCamera;
    this->update();
}

void VRWidget::changeBounce()
{
    bounceAnimation = !bounceAnimation;
}

void VRWidget::targetOnPlanet()
{
...
void VRWidget::initializeGL()
{
    ...
    plane->setPosition(QVector3D(0, -2, -4));
    plane->setTexture(":/textures/wood.png");
    plane->initialize();

    trajectory = new VRBounce();
    //trajectory->setPosition(QVector3D(0, -4, 0));
    //trajectory->initialize();


    world = new VRPlanet(2);
...
/*
    ...
    torus = new VRTorus();
    torus->setPosition(QVector3D(0, 5, -10));
...
void VRWidget::paintGL()
{
    ...
    skybox->draw();
    if (stereo){
        glColorMask(true, false, false, false); // filtre rouge
        camera->setCameraType(VR_CAMERA_LEFT);
        scene->draw();

        glClear(GL_DEPTH_BUFFER_BIT) ;

        glColorMask(false, true, true, false);  // filtre cyann
        camera->setCameraType(VR_CAMERA_RIGHT);
        scene->draw();

        glColorMask(true,true,true,false); // normal
    }
    else {
        scene->draw();
    }
